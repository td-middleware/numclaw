...
var OrderedListItemContext = import_react3.createContext({ marker: "" });
function OrderedListItem({
  children
}) {
  const { marker } = import_react3.useContext(OrderedListItemContext);
...
var OrderedListContext = import_react4.createContext({ marker: "" });
function OrderedListComponent({ children }) {
  const { marker: parentMarker } = import_react4.useContext(OrderedListContext);
  let numberOfItems = 0;
