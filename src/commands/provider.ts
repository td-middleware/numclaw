...
const call: LocalCommandCall = async (args, context) => {
  ...
  if (arg === 'unset') {
    ...
    delete process.env.CLAUDE_CODE_USE_LOCAL
    return {
      type: 'text',
  ...
  if (arg === 'local') {
    ...
    if (!hasUrl) {
      ...
      return {
        type: 'text',
        value: `Switched to Local provider.\nWarning: Missing env var: LOCAL_BASE_URL\nConfigure it via /login or set manually.\nExample: LOCAL_BASE_URL=http://localhost:11434/v1`,
      }
    }
  }

  // Handle different provider types
  // - 'anthropic', 'openai', 'gemini', 'grok', 'local' are stored in settings.json (persistent)
  // - 'bedrock', 'vertex', 'foundry' are env-only (do NOT touch settings.json)
  if (arg === 'anthropic' || arg === 'openai' || arg === 'gemini' || arg === 'grok' || arg === 'local') {
